...
import static com.smartloan.smtrick.smart_loan_user.constants.Constant.STATUS_DOCPICKUP;

public class Fragment_Dock_pickup_Leeds extends Fragment {
    ...
    public Fragment_Dock_pickup_Leeds() {
    }
    ...
    private void filterList(ArrayList<LeedsModel> invoiceArrayList) {
        ...
        if (invoiceArrayList != null) {
            for (LeedsModel invoice : invoiceArrayList) {
                if (!Utility.isEmptyOrNull(invoice.getStatus()) && invoice.getStatus().equalsIgnoreCase(STATUS_DOCPICKUP))
